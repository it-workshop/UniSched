...
A: На данный момент модули могут быть двух типов:
...
Q: Я написал новый модуль, что мне сделать, чтобы он собирался опционально?

A: добавить в корневой CMakeLists.txt строчку типа:

    option(WITH_MEGAMODULE "Собирать супер-модуль" OFF)

Собирать с этим модулем:

    cmake -WITH_MEGAMODULE=ON

Если модуль хорошо отлажен, можно выставить Y вместо N, тогда будет собираться по умолчанию. Если модуль имеет слабое отношение к ядру, хорошо бы его включить в список отключаемых модулей параметром CORE_ONLY. Тогда строка с опцией может выглядеть, например, так:

    CMAKE_DEPENDENT_OPTION(WITH_MEGAMODULE "Собирать супер-модуль" ON "NOT CORE_ONLY" OFF)
